...
python manage.py seed --count 10
```

## CSV Import

Use the importer command:

```bash
python manage.py import_partner_csv template.csv --dry-run
python manage.py import_partner_csv template.csv
```

You can also use the UI:

- Go to Community Partners list
- Click **Bulk Import CSV**
- Download the template from the import page

## Notes

- Archive behavior is used instead of hard delete for partners, needs, and interventions.
- For multiline CSV fields, wrap values in double quotes.
